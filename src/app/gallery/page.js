...
export default function Gallery() {
  ...
  const fetchCreations = async () => {
    try {
      ...
      console.error("Error fetching pet creations:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = async (imageUrl, filename) => {
    ...
    try {
      ...
      a.href = url;
      a.download = filename || `pet-product-ad-${Date.now()}.png`;
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 bg-transparent">
        <div className="max-w-md text-center space-y-6">
          ...
          <p className="text-secondary-text text-sm leading-relaxed">
            Please sign in to view your collection of generated AI pet product ad images.
          </p>
          <button
    ...
    <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full bg-transparent overflow-y-auto scrollbar-subtle">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-divider pb-6 mb-8 gap-4">
        <div>
          ...
          <p className="text-xs text-secondary-text mt-1 font-medium">A portfolio of all your AI generated pet product ad images.</p>
        </div>
      ...
      {creations.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-16 text-center border border-divider rounded-2xl bg-bg-card/20">
          ...
          <span className="text-sm font-bold text-primary-text">No ad photos generated yet</span>
          <p className="text-xs text-secondary-text mt-2 max-w-[200px] leading-relaxed">
            Head to the workspace editor and start generating your custom pet product ad concepts.
          </p>
        </div>
      ) : (
          ...
          >
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  onClick={() => handleDownload(activeCreation.outputUrl, `pet-product-ad-${activeCreation.id}.png`)}
